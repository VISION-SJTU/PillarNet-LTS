...
def points_in_boxes_2d_gpu(points, boxes):
    """ check points in boxes2d in X-Y planes.
    :param points: (B, M, 2)
    ...
    # cp_boxes = boxes.clone()
    boxes = to_pcdet(boxes)
    box_idxs_of_pts = points.new_zeros((batch_size, num_points), dtype=torch.int).fill_(-1)
    roiaware_pool3d_cuda.points_in_boxes_2d_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)

    # from tools.visual import draw_boxes, draw_points
    # viser = draw_boxes(cp_boxes[:1].view(-1, 7).cpu().numpy())
    # pts = points[0]
    # pts = torch.cat([pts, pts.new_zeros((len(pts), 1))], dim=1)
    # fg_pts = pts[box_idxs_of_pts[0] >= 0]
    # viser = draw_points(fg_pts.cpu().numpy(), viser=viser, point_size=3)
    # viser.run()
