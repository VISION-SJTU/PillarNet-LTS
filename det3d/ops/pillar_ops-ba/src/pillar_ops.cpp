...
int create_pillar_indices_stack_wrapper(float bev_size, at::Tensor xy_tensor, at::Tensor xy_batch_cnt_tensor,
                                        at::Tensor pillar_mask_tensor) {
	CHECK_INPUT(xy_tensor);
	CHECK_INPUT(xy_batch_cnt_tensor);
    CHECK_INPUT(pillar_mask_tensor);

    int N = xy_tensor.size(0);
    int C = xy_tensor.size(1);
	assert(C == 2);
    int B = pillar_mask_tensor.size(0);
    int H = pillar_mask_tensor.size(1);
    int W = pillar_mask_tensor.size(2);

    const int *xy_batch_cnt = xy_batch_cnt_tensor.data_ptr<int>();
    const float *xy = xy_tensor.data_ptr<float>();
    bool *pillar_mask = pillar_mask_tensor.data_ptr<bool>();

    create_pillar_indices_stack_kernel_launcher(N, B, H, W, bev_size, xy, xy_batch_cnt, pillar_mask);
...
int create_pillar_indice_pairs_stack_wrapper(float bev_size, at::Tensor xy_tensor, at::Tensor xy_batch_cnt_tensor,
    ...
    create_pillar_indice_pairs_stack_kernel_launcher(N, B, H, W, bev_size, xy, xy_batch_cnt,
