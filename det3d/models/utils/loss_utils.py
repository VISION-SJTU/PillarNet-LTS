...
def _reg_loss(regr, gt_regr, mask, iou=False):
    """
    Refer to https://github.com/tianweiy/CenterPoint
    L1 regression loss
    Args:
    ...
    if iou:
        gt_regr = boxes_aligned_iou3d_gpu(regr, gt_regr)
        gt_regr = 2 * gt_regr - 1

    loss = F.l1_loss(regr, gt_regr, reduction='none')
...
class IoULossCenterNet(nn.Module):
    ...
    def forward(self, output, mask, ind=None, target=None):
        ...
        if ind is None:
            ...
            pred = _transpose_and_gather_feat(output, ind)
        loss = _reg_loss(pred, target, mask, iou=True)
        return loss
