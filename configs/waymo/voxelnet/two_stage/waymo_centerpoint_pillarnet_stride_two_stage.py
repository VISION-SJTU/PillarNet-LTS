...
model = dict(
    ...
    first_stage_cfg=dict(
        ...
        reader=dict(type="Identity", pc_range=pc_range, num_input_features=2),
        backbone=dict(
            type="SpMiddlePillarEncoderCSP18", num_input_features=2, ds_factor=8, double=2,
            pc_range=pc_range,
            pillar_cfg=dict(
                pool0=dict(bev=0.1 / 2),
                pool1=dict(bev=0.1),
                pool2=dict(bev=0.1 * 2),
                pool3=dict(bev=0.1 * 4),
            ),
        ),
        neck=dict(
            type="RPNG2",
            ...
            us_num_filters=[128, 128, 64],  # [128, 128, 64]
    ...
    second_stage_modules=[
        dict(
            type="BEVFeatureStrideNeck",
            bev_channels=128,
            feature_sources=['x_conv3'],
...
assigner = dict(
    ...
    max_objs=500,
    min_radius=2,  # corresponding to classes
    pc_range=pc_range,
    voxel_size=[*pillar_size, 6],
)
